...
ARGUMENTS = [
    ...
    DeclareLaunchArgument(
        "initial_pose_x",
        default_value="0.5",
        description="Initial x pose of rasbot in the simulation.",
    ),
    DeclareLaunchArgument(
        "initial_pose_y",
        default_value="0.0",
        description="Initial y pose of rasbot in the simulation.",
    ),
    DeclareLaunchArgument(
        "initial_pose_z",
        default_value="0.0",
        description="Initial z pose of rasbot in the simulation.",
    ),
    DeclareLaunchArgument(
        "initial_pose_yaw",
        default_value="0.0",
        description="Initial yaw pose of rasbot in the simulation.",
    ),
    DeclareLaunchArgument(
    ...
    DeclareLaunchArgument(
        "entity", default_value="rby", description="Name of the robot"
    ),
]
...
def get_robot_description():
    ...
    robot_description_path = os.path.join(
        gazebo_rby, "urdf", "robots", "rby_gazebo.urdf.xacro"
    )
    ...
    robot_desc = robot_desc.replace("package://urdf_rby/", f"file://{pkg_urdf_rby}/")
    return robot_desc
...
def generate_launch_description():
    ...
    controller_config_path = os.path.join(
        gazebo_rby, "config", "rby_controller_manager.yaml"
    )

    bridge_config_file_path = os.path.join(gazebo_rby, "config", "ros_gz_bridge.yaml")
    controllers_config_file_path = os.path.join(
        gazebo_rby, "config", "controllers.yaml"
    )

    set_env_vars_resources = AppendEnvironmentVariable(
        "GZ_SIM_RESOURCE_PATH", os.path.join(gazebo_rby, "models")
    )
    ...
    rby_controller_spawner_head = Node(
        package="controller_manager",
        executable="spawner",
        arguments=[
    ...
    rby_controller_spawner_right_arm = Node(
        package="controller_manager",
        executable="spawner",
        arguments=[
    ...
    rby_controller_spawner_left_arm = Node(
        package="controller_manager",
        executable="spawner",
        arguments=[
    ...
    rby_controller_spawner_left_gripper = Node(
        package="controller_manager",
        executable="spawner",
        arguments=[
    ...
    rby_controller_spawner_right_gripper = Node(
        package="controller_manager",
        executable="spawner",
        arguments=[
    ...
    rby_controller_spawner_torso = Node(
        package="controller_manager",
        executable="spawner",
        arguments=[
    ...
    rby_controller_spawner_wheel = Node(
        package="controller_manager",
        executable="spawner",
        arguments=[
            "joint_trajectory_controller_wheel",
            "--param-file",
            controller_config_path,
        ],
    )
    ...
    ld.add_action(
        RegisterEventHandler(
            event_handler=OnProcessExit(
                ...
                on_exit=[rby_controller_spawner_head],
            )
        )
    )
    ...
    ld.add_action(
        RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=joint_state_broadcaster_spawner,
                on_exit=[rby_controller_spawner_right_arm],
            )
        )
    )
    ...
    ld.add_action(
        RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=joint_state_broadcaster_spawner,
                on_exit=[rby_controller_spawner_left_arm],
            )
        )
    )
    ...
    ld.add_action(
        RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=joint_state_broadcaster_spawner,
                on_exit=[rby_controller_spawner_left_gripper],
            )
        )
    )
    ...
    ld.add_action(
        RegisterEventHandler(
            event_handler=OnProcessExit(
                target_action=joint_state_broadcaster_spawner,
                on_exit=[rby_controller_spawner_right_gripper],
            )
        )
    )
